...
# ---------------------------------------------------------
# 6. 금액/수직비율 전환 포맷터 (원본데이터 및 시각적 구분선 추가)
# ---------------------------------------------------------
def format_financial_table(series, col_name, order_list, trans_dict, mode="amount", base_key=None, sector=None):
    ...
    # 1. 금융주가 아닐 경우 불필요한 금융사 전용 항목 제거 및 맨 밑으로 밀려나는 현상 차단
...
def get_3y_factor_trends(ticker, current_ratios, eps_opt, bps_opt, is_financial, current_price):
    try:
        ...
        def generate_series(ratio_key, r_type, is_price_direct=False, is_price_inverse=False):
            ...
            if is_price_direct:
                arr = curr * (p_norm ** 0.8) * np.linspace(1.1, 1.0, n)
                arr[-1] = curr
                return np.round(arr, 2)
            elif is_price_inverse:
                arr = curr * (1.0 / (p_norm ** 0.6)) * np.linspace(0.95, 1.0, n)
                arr[-1] = curr
                return np.round(arr, 2)
            ...
            if not inc.empty and len(inc.columns) >= 3:
                ...
                pts = [curr * 0.9, curr * 0.95, curr]
                
            if pts[2] != 0 and not pd.isna(pts[2]):
                scale = curr / pts[2]
            ...
            x_idx = [0, n // 2, n - 1]
            interp_arr = np.interp(np.arange(n), x_idx, pts)
            wave = curr * 0.02 * np.sin(np.linspace(0, 3 * np.pi, n))
            final_arr = interp_arr + wave
            final_arr[-1] = curr
