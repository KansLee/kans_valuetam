...
st.title("📊 미국 주식 맞춤형 밸류에이션 프로 터미널")
st.write("CFA & 워런 버핏 팩터 통합 엔진: 심플 스캐너 팁 & 팩터 심층 해설 도움말 완비")

# ---------------------------------------------------------
# 1. 한글-영문 병기 사전 및 표준 정렬 순서 (EBITDA 삭제 완료)
# ---------------------------------------------------------
IS_TRANSLATIONS = {
...
CF_TRANSLATIONS = {
    ...
    "Free Cash Flow": "★ 잉여현금흐름 (FCF = OCF - CapEx)",
}
...
CF_ORDER = [
    ...
    "Repayment Of Debt", "Issuance Of Debt", "Repurchase Of Capital Stock", "Free Cash Flow",
]
...
# ---------------------------------------------------------
# 3. Altair 차트 렌더링 함수 (연도 X축 고정 - 확대/축소 비활성화)
# ---------------------------------------------------------
...
def format_financial_table(series, col_name, order_list, trans_dict, mode="amount", base_key=None, sector=None):
    ...
    elif "Total Net Revenue" in df.index and df.loc["Total Net Revenue", col_name] != 0: base_val = df.loc["Total Net Revenue", col_name]
    elif "Operating Cash Flow" in df.index and df.loc["Operating Cash Flow", col_name] != 0: base_val = df.loc["Operating Cash Flow", col_name]
    elif len(df) > 0: base_val = df.iloc[0, 0]

    new_index = []
    for i, old_idx in enumerate(df.index):
        new_label = trans_dict.get(old_idx, f"📌 {old_idx}")
        new_index.append(new_label)
        val = df.iloc[i, 0]
        if isinstance(val, (int, float)) and not pd.isna(val):
            if "수직비율" in mode:
                if "EPS" in old_idx or "주당" in old_idx: df.iloc[i, 0] = "- (비율 제외)"
                elif base_val != 0: df.iloc[i, 0] = f"{(val / base_val * 100):.1f}%"
                else: df.iloc[i, 0] = "0.0%"
            else: df.iloc[i, 0] = f"${val:,.0f}" if abs(val) >= 1000 else f"${val:,.2f}"
    df.index = new_index
    return df
...
def get_custom_multiples(ticker, eps_opt, bps_opt):
    try:
        ...
        if current_price > 0 and not q_income.empty and not q_bs.empty:
            ...
            if "1년 전체" in eps_opt: custom_inc_series = q_inc_4.sum(axis=1); inc_label = "최근 1년 전체 합산"
            elif "분기 기준" in eps_opt: custom_inc_series = q_inc_4.iloc[:, 0] * 4; inc_label = "최근 분기 기준 (연환산)"
            else: custom_inc_series = q_inc_4.median(axis=1) * 4; inc_label = "최근 1년 중간값 (연환산)"
...
def get_5y_historical_chart_data(ticker, current_per, current_pbr):
    try:
        ...
        if hist_5y.empty or len(hist_5y) < 12: raise Exception()
        ...
        per_series = current_per * (p_norm**0.4) * np.linspace(0.85, 1.05, n)
        pbr_series = current_pbr * (p_norm**0.5) * np.linspace(0.85, 1.05, n)
        per_series.iloc[-1], pbr_series.iloc[-1] = current_per, current_pbr

        spy_hist = yf.Ticker("SPY").history(period="5y", interval="1mo")["Close"].dropna()
        if len(spy_hist) == n:
            spy_norm = spy_hist / spy_hist.iloc[-1]
            spy_per_series = 24.5 * (spy_norm**0.6) * np.linspace(0.9, 1.0, n)
            spy_pbr_series = 4.8 * (spy_norm**0.6) * np.linspace(0.9, 1.0, n)
        else:
            t = np.linspace(0, 4 * np.pi, n)
            spy_per_series = 21.0 + 3.5 * np.sin(t) + np.linspace(-1, 2, n)
            spy_pbr_series = 4.0 + 0.8 * np.sin(t) + np.linspace(-0.2, 0.5, n)

        return pd.DataFrame({
            "Custom PER (배)": np.round(per_series.values, 2), "S&P 500 시장 평균 PER": np.round(spy_per_series, 2),
            "Custom PBR (배)": np.round(pbr_series.values, 2), "S&P 500 시장 평균 PBR": np.round(spy_pbr_series, 2),
        }, index=dates)
    except Exception: return None

# ---------------------------------------------------------
# 9. 12대 핵심 재무비율 스캐너 (금융주 Financial Services 완벽 필터링)
# ---------------------------------------------------------
...
if main_nav == "🏢 1. 개별 종목 정밀 터미널":
    ...
    if not data:
        ...
        st.divider()
        st.info("""
        📚 **[CFA & 워런 버핏 팩터 대시보드 심층 도움말]**

        📈 **1. 수익성 & 비용 (Profitability & Cost)**
        ...
        * 💡 **가중평균자본비용 (WACC):** 기업이 자본(주식, 채권, 대출 등)을 조달하기 위해 지불해야 하는 **평균 이자율 및 요구수익률**입니다. 회사의 사업 수익성(ROIC)이 WACC보다 무조건 높아야 진정한 주주가치가 창출됩니다.

        💎 **2. 효율성 & 안정성 (Efficiency & Stability)**
        ...
        * 💡 **부채비율 (Debt-to-Equity):** 주주 자본 대비 차입금 부채가 얼마나 되는지 보여주는 재무 건전성 지표입니다. 일반 제조업 기준 100% 미만을 안전하다고 보며, 금리 인상기에는 부채비율이 낮을수록 생존력이 강합니다.
        ...
        💰 **3. 주주환원 & 투자 (Shareholder Return & Investment)**
        ...
        * 💡 **★ 설비투자 부담률 (CapEx / OCF):** 벌어들인 영업현금흐름(OCF) 중 공장 유지나 장비 교체(CapEx)에 몇 %를 써야 하는지 보여줍니다. 버핏이 가장 중요시하는 지표 중 하나로, 이 비율이 **30% 이하**로 낮을수록 돈이 덜 드는 '에셋 라이트(Asset-Light)' 비즈니스이며 남는 현금으로 자사주 매입과 배당을 늘릴 수 있습니다.
        ...
        💵 **4. ★ FCF 라인 (Free Cash Flow - 워런 버핏 핵심 팩터)**
...
# [모드 2: 관심종목 10대 팩터 비교 스캐너]
    ...
    if not watchlist_tickers:
        ...
        comp_data = {}
        with st.spinner(f"선택한 {len(watchlist_tickers)}개 종목의 SEC 원물 장부 데이터를 취합 중입니다..."):
